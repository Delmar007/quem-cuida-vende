...
export default function AdminFranqueado() {
  ...
  const tabs = [
    ...
    { id: 'veiculos', label: 'Veículos do Site', type: 'crm' },
  ...
  useEffect(() => {
    ...
    localStorage.removeItem('delmar_franqueado_auth');
    
    // Check Auth
    if (sessionStorage.getItem('delmar_franqueado_auth') === 'true') {
  ...
  useEffect(() => {
    if (sessionStorage.getItem('delmar_franqueado_auth') === 'true') {
      if (['veiculos', 'matches'].includes(activeTab)) {
        loadCRMData();
      }
    }
  }, [activeTab]);
      ...
      <div style={{ backgroundColor: '#000', backgroundImage: 'url("/cabeçalho_colmeia.png.png")', backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ backgroundColor: 'var(--surface)', padding: '40px', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-md)', width: '100%', maxWidth: '400px', textAlign: 'center' }}>
          ...
          <input 
            ...
            onKeyDown={e => {
              if (e.key === 'Enter') {
                const storedPass = localStorage.getItem('delmar_franqueado_password') || 'franqueado123';
                if (password === storedPass) {
                  sessionStorage.setItem('delmar_franqueado_auth', 'true');
                  setIsAuthenticated(true);
                  loadCRMData();
                } else {
          ...
          <button 
            ...
            style={{ width: '100%', padding: '12px', fontSize: '1.1rem', fontWeight: '600', marginBottom: '15px' }}
          >
            Entrar
          </button>
          <button 
  ...
  return (
    <div className="admin-layout">
      ...
      <div className="admin-sidebar">
        <h2 className="admin-title" style={{ fontSize: '1.2rem', fontWeight: '700', marginBottom: '20px' }}>Painel Franqueado</h2>
        ...
        <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <button 
            onClick={() => {
              sessionStorage.removeItem('delmar_franqueado_auth');
      ...
      <div className="admin-editor-area">
        ...
        {activeTab === 'moderacao' && (
          <div style={{ maxWidth: '940px', margin: '0 auto' }}>
            <div className="admin-header">
              <h3 className="admin-subtitle">Aprovação de Anúncios Pendentes</h3>
            </div>
                      ...
                      return (
                        <tr key={car.id}>
                          ...
                          <td style={{ maxWidth: '400px' }}>
                            <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '8px' }}>
                                {carImgsSafe.map((img, idx) => (
                                  <div key={idx} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: '160px' }}>
                          ...
                          <td style={{ textAlign: 'right', minWidth: '150px' }}>
                            <button onClick={() => approveCar(car)} className="btn btn-primary" style={{ backgroundColor: '#16a34a', borderColor: '#16a34a', padding: '6px 12px', fontSize: '0.85rem', marginBottom: '8px', width: '100%' }}>
                              ✅ Aprovar
                            </button>
                            <a href={`/vender?edit=${car.id}`} target="_blank" rel="noreferrer" className="btn btn-outline" style={{ display: 'inline-block', padding: '6px 12px', fontSize: '0.85rem', width: '100%', textAlign: 'center', textDecoration: 'none' }}>
                              ✏️ Editar Anúncio
                            </a>
                          </td>
                        </tr>
                  ...
                  {filteredCars.map(car => {
                    ...
                    return (
                      <tr key={car.id} style={{ borderBottom: '1px solid var(--border)' }}>
                        ...
                        {/* COLUNA PROPRIETARIO / PARCEIRO (Mascarada para o Franqueado) */}
                        <td style={{ padding: '16px', verticalAlign: 'top' }}>
                          <div>
                            <div style={{ paddingBottom: '0', borderBottom: 'none', marginBottom: '0' }}>
                              <div style={{ fontSize: '0.75rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                                Proprietário
                              </div>
                              <div style={{ fontWeight: '600', color: 'var(--text-primary)' }}>Delmar de Oliveira</div>
                              
                              <div style={{ color: 'var(--text-secondary)', marginTop: '4px' }}>
                                <a href={`https://wa.me/5567991694802`} target="_blank" rel="noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', color: '#25D366', textDecoration: 'none', fontWeight: '600', padding: '2px 6px', backgroundColor: '#eefcf3', borderRadius: '4px', fontSize: '0.65rem' }}>
                        ...
                        <td style={{ padding: '16px', verticalAlign: 'top', textAlign: 'center' }}>
                          {(() => {
                            let isPostedByFranqueado = false;
                            try {
                              const detObj = JSON.parse(car.detalhes || '{}');
                              isPostedByFranqueado = detObj.postedBy === 'franqueado';
                            } catch(e) {}

                            if (!isPostedByFranqueado) {
                              return (
        ...
        {activeTab === 'cadastrar-anuncio' && (
          <div className="admin-form-container">
            <ErrorBoundary>
              <AdminAddCar setActiveTab={setActiveTab} isFranqueado={true} />
            </ErrorBoundary>
          </div>
        )}
          ...
          <div style={{ maxWidth: '940px', margin: '0 auto' }}>
            ...
            <div style={{ backgroundColor: '#fff', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '24px' }}>
              ...
              <button 
                className="btn btn-primary"
                onClick={() => {
                  const newPassword = document.getElementById('newFranqueadoPassword').value;
                  if (newPassword.length < 4) {
                    alert('A senha deve ter pelo menos 4 caracteres.');
                    return;
                  }
                  localStorage.setItem('delmar_franqueado_password', newPassword);
                  alert('Senha atualizada com sucesso!');
                  document.getElementById('newFranqueadoPassword').value = '';
                }}
              >
                Salvar Nova Senha
              </button>
